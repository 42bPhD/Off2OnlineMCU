...
void AdaptiveAvgPool2d_q7_HWC(q7_t* Im_in,
    const uint16_t dim_im_in,//3
    const uint16_t ch_im_in,//128
    const uint16_t dim_im_out,//1
    q7_t* bufferA,
    q7_t* Im_out)
{
    const uint16_t stride = dim_im_in / dim_im_out;
    const uint16_t dim_kernel = dim_im_in / dim_im_out;
    /* Run the following code as reference implementation for Cortex-M0 and Cortex-M3 */
    ...
    for (int i_ch_in = 0; i_ch_in < ch_im_in; i_ch_in++) {
        for (int i_y = 0; i_y < dim_im_out; i_y++) {
            for (int i_x = 0; i_x < dim_im_out; i_x++) {
                int sum = 0;
                for (int k_y = i_y * stride; k_y < i_y * stride + dim_kernel; k_y++) {
                    for (int k_x = i_x * stride; k_x < i_x * stride + dim_kernel; k_x++) {
                        sum += Im_in[i_ch_in + ch_im_in * (k_x + k_y * dim_im_in)];
                    }
                }
                Im_out[i_ch_in + ch_im_in * (i_x + i_y * dim_im_out)] = sum / (dim_kernel * dim_kernel);
            }
        }
    }
