...
def calculate_fractional_offset(input_tensor, bitwidth=8):
    """Calculate the fractional offset for the fixed-point representation
    with a bitwidth of 8 given an input tensor such that no values are clipped.
    
    Args:
        input_tensor (tensor): Input tensor of values
    ...
    sign_bit = 1
    largest_value = torch.max(torch.abs(input_tensor))
    int_bits = torch.ceil(torch.log2(largest_value))
...
def find_offset_for_layer_weights(model_obj, layer_name, bitwidth, parameter_type):
    ...
    for name, module in model_obj.model.named_modules():
        if name == layer_name:
            if parameter_type == 0:
                return calculate_fractional_offset(module.weight.data, bitwidth)
            else:
                return calculate_fractional_offset(module.bias.data, bitwidth)
...
def find_offsets_for_model_weights(model_obj, parameter_type, bitwidth):
    ...
    for name, module in model_obj.model.named_modules():
        if isinstance(module, (nn.Conv2d, nn.Linear)):
            if parameter_type == 0:
                fractional_offsets[name] = calculate_fractional_offset(module.weight.data, bitwidth)
            else:
                if module.bias is not None:
                    fractional_offsets[name] = calculate_fractional_offset(module.bias.data, bitwidth)
...
def evaluate_quantized_weights(model_obj, original_acc, bitwidth=8):
    ...
    new_acc = model_obj.evaluate_accuracy()
    print(f'Quantized network\'s accuracy: {new_acc}')
    print(f'Bandwidth: {bitwidth}')
...
def find_offset_for_layer_activations(model_obj, layer_name, bitwidth, num_images=250):
    ...
    for name, module in model_obj.model.named_modules():
        if (name.startswith('conv2d') or name.startswith('dense') or name.startswith('binary')) and (layer_name == name):
            act_maps = model_obj.get_activation_maps(model_obj.x_test[:num_images], name)
...
def find_offsets_for_model_activations(model_arch, name, test_data, bitwidth, num_images=250):
    ...
    Args:
        ...
        test_data (sequence): Test data needed for inference. Sequence of tensors
        bitwidth (int): Bitwidth of the targeted fixed-point representation
    ...
    for name, module in model_obj.model.named_modules():
        if name.startswith('conv2d') or name.startswith('dense') or name.startswith('binary'):
            act_maps = model_obj.get_activation_maps(model_obj.x_test[:num_images], name)
            fractional_offsets[name] = calculate_fractional_offset(act_maps, bitwidth)
...
def evaluate_quantized_activations(model_arch, name, test_data, original_acc, bitwidth=8, num_images=250):
    ...
    new_acc = quant_model.evaluate_accuracy()
    print(f'Quantized network\'s accuracy: {new_acc}')
    print(f'Accuracy drop: {((original_acc - new_acc)/original_acc * 100):.3f} %')
