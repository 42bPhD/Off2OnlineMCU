...
from qtimm.util import accuracy, AverageMeter, load_weights
from qtimm.loss import SoftTargetCrossEntropy, LabelSmoothingCrossEntropy
...
from utils import (load_checkpoint, save_checkpoint, get_grad_norm, 
                   auto_resume_helper,
                   reduce_tensor, save_latest, 
                   update_model_ema, unwrap_model)
import copy
...
def parse_option():
    ...
    # parser.add_argument('--arch', default="RepVGG-C1", type=str, help='arch name')
    parser.add_argument('--arch', default="RepVGG_MobileNetV1", type=str, help='arch name')
    parser.add_argument('--batch-size', default=128, type=int, help="batch size for single GPU")
    parser.add_argument('--data-path', default='E:/1_TinyML/tiny/benchmark/training/visual_wake_words/vw_coco2014_96/', type=str, help='path to dataset')
    ...
    parser.add_argument('--output', default='./trained_models', type=str, metavar='PATH',
...
def get_cifar10_loader(data_root='E:/2_Quantization/torch2cmsis/examples/cifar/data/data_cifar10',
                        batchsize=100,
                       num_workers=2,
                       image_size=32):
        ...
        trainloader = torch.utils.data.DataLoader(train_dataset, batch_size=batchsize, shuffle=True, num_workers=num_workers)
        ...
        testloader = torch.utils.data.DataLoader(test_dataset, batch_size=batchsize, shuffle=False, num_workers=num_workers)
...
def main(config):
    ...
    # VWW
    data_loader_train, data_loader_val = get_dataloader(dataset_dir=config.DATA.DATA_PATH,
                                                        batch_size=config.DATA.BATCH_SIZE,
                                                        num_workers=config.DATA.NUM_WORKERS,
                                                        image_size=config.DATA.IMG_SIZE)
    # Ciar10
    # data_loader_train, data_loader_val = get_cifar10_loader(batchsize=config.DATA.BATCH_SIZE,
    #                                                         num_workers=config.DATA.NUM_WORKERS,
    #                                                         image_size=config.DATA.IMG_SIZE)
    # setup mixup / cutmix
    # mixup_fn = None
    ...
    # model = LeNet(num_channels=3, num_classes=10, model='arm', qat=False)
    # model = create_RepVGGplus_by_name(config.MODEL.ARCH, deploy=False, use_checkpoint=args.use_checkpoint, num_classes=config.MODEL.NUM_CLASSES)
    '''
    from torchsummary import summary
    from torchprofile import profile_macs
    # print(summary(model, (3, 32, 32), device='cpu'))
    from repvgg import repvgg_model_convert, create_RepVGG_A0, create_RepVGG_C1, create_RepVGG_LeNet
    # train_model = create_RepVGG_A0(deploy=False, num_classes=1000)
    train_model = create_RepVGG_LeNet(deploy=False, num_classes=10)
    # train_model.load_state_dict(torch.load('RepVGG-A0-train.pth'))
    # print(summary(train_model, (3, 32, 32), device='cpu'))
    deploy_model = repvgg_model_convert(train_model, save_path='RepVGG-C1-deploy.pth')
    
    # print(f'{profile_macs(train_model, torch.randn(1, 3, 32, 32)):,}')
    # print(f'{profile_macs(deploy_model, torch.randn(1, 3, 32, 32)):,}')
    print(summary(deploy_model, (3, 32, 32), device='cpu'))
    exit()
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, model_ema=None):
    ...
    batch_time = AverageMeter('Time', fmt=':.4f')
    loss_meter = AverageMeter('Loss', ':.4e')
    norm_meter = AverageMeter('grad_norm', ':.4f')
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if mixup_fn is not None and len(samples) %2 == 0:
        ...
        if model_ema is not None:
            update_model_ema(config, num_gpus=1, model=model, model_ema=model_ema, cur_epoch=epoch, cur_iter=idx)
...
def validate(config, data_loader, model):
    ...
    acc5_meter = AverageMeter('Acc@2', ':6.2f')
