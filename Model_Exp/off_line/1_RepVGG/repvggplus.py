...
# from quantization.qformat import QConv2d, QLinear
Conv2d = QConv2d
Linear = QLinear
# ReLU = ReLU6
...
def conv_bn_relu(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    ...
    relu = ReLU()
    conv, bn, relu = fold_CBR(conv, bn, relu)
    result.add_module('conv', conv)
    result.add_module('bn', bn)
    result.add_module('relu', relu)
...
class RepVGGplusBlock(nn.Module):
    ...
    def switch_to_deploy(self):
        ...
        self.rbr_reparam = Conv2d(in_channels=self.rbr_dense.conv.in_channels,
...
class RepVGGplus(nn.Module):
    ...
    def _build_aux_for_stage(self, stage):
        ...
        fc = Linear(stage_out_channels, self.num_classes, bias=True)
...
def create_RepVGGplus_L2pse(deploy=False, use_checkpoint=False, num_classes=1000):
    return RepVGGplus(num_blocks=[8, 14, 24, 1], num_classes=num_classes,
...
def create_RepVGGplus_by_name(name, deploy=False, use_checkpoint=False, num_classes=2):
    if 'plus' in name:
        return repvggplus_func_dict[name](deploy=deploy, use_checkpoint=use_checkpoint, num_classes=num_classes)
    else:
        print('=================== Building the vanila RepVGG ===================')
        from repvgg import get_RepVGG_func_by_name
        return get_RepVGG_func_by_name(name)(deploy=deploy, use_checkpoint=use_checkpoint, num_classes=num_classes)
