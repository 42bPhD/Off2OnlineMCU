...
class Conv2d(nn.Conv2d):
    ...
    def quant_answer(self):
        self.weight.data = qsigned(self.weight, self.weight_log2_t, self.weight_bit_width)**(self.weight_bit_width - 1 - torch.ceil(self.weight_log2_t)).int()
        self.bias.data = qsigned(
            self.bias, self.bias_log2_t, self.bias_bit_width)**(self.bias_bit_width - 1 -torch.ceil(self.bias_log2_t)).int()
    ...
    def forward(self, input):
        return self.conv_forward(input) if self.quant else self.conv_forward_unquant(input)
